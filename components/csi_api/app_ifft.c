/*
 * 复用 CSI 示例中的 FFT 与复数运算，用于从 CSI 计算 CIR/相位
 *
 * 源自：csi/esp-csi-ext_for_c5c6/examples/1124echoear/master_recv/main/app/app_ifft.c
 * 做了极少量注释性改动，逻辑保持一致，方便在 Brookesia 应用中直接使用。
 */
...
// 固定 64 点 FFT 的 bit-reverse 表
static const int bitReverseTable[64] = {
...
void fft_iq(Complex_Iq *X, int inverse)
{
    int log2N = 6;
    int N = 64;
...
static unsigned int inline bitReverse(unsigned int x, int log2n)
{
    int n = 0;
...
void fft(Complex *X, int N, int inverse)
{
    ...
    for (int s = 1; s <= log2N; ++s) {
        ...
        w.imag = 0.0f;
        Complex wm;
        wm.real = cosf(-2.0f * PI / (float)m);
        wm.imag = sinf(-2.0f * PI / (float)m);
        if (inverse) {
            wm.imag = -wm.imag;
        }
